...
angular.module('scannabis')
	.directive('uploader', function ($rootScope, $sce, $firebaseArray, $firebaseObject, Data) {
		...
		return {
			...
			template: '<form id="{{name}}-form" novalidate><input class="well" type="file" id="{{name}}" ng-blur="upload()"/><button type="button" class="tertiary" data-ng-click="upload()">Upload</button></form>',
			restrict: 'E',
